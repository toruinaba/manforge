...
def test_isotropic_C_shapes():
    ...
    for ss, expected_shape in [
        (SOLID_3D, (6, 6)),
        (PLANE_STRAIN, (4, 4)),
        (PLANE_STRESS, (3, 3)),
        (UNIAXIAL_1D, (1, 1)),
    ]:
        model = J2IsotropicHardening(ss)
        C = model.elastic_stiffness(params)
        assert C.shape == expected_shape, f"{ss.name}: expected {expected_shape}, got {C.shape}"
...
def test_isotropic_C_1d():
    ...
    from manforge.models.j2_isotropic import J2IsotropicHardening
    E, nu = 200e3, 0.3
    model = J2IsotropicHardening(UNIAXIAL_1D)
    C = model.elastic_stiffness({"E": E, "nu": nu, "sigma_y0": 250.0, "H": 0.0})
...
def test_plane_stress_C_symmetry():
    ...
    from manforge.models.j2_isotropic import J2IsotropicHardening
    model = J2IsotropicHardening(PLANE_STRESS)
