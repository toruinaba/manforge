...
def test_bauschinger_effect(model):
    ...
    _r3 = return_mapping(model, small_compressive_plastic, sigma2, state2)
...
def test_plane_strain_yield_consistency():
    ...
    _r = return_mapping(model_pe, deps, jnp.zeros(4), state0)
    stress_new, state_new = _r.stress, _r.state
...
def test_plane_stress_yield_consistency():
    ...
    _r = return_mapping(model_ps, deps, jnp.zeros(3), state0)
    stress_new, state_new = _r.stress, _r.state
...
def test_1d_yield_consistency():
    ...
    state0 = model_1d.initial_state()
    deps = jnp.array([3e-3])
    _r = return_mapping(model_1d, deps, jnp.zeros(1), state0)
    stress_new, state_new = _r.stress, _r.state
...
def test_1d_bauschinger():
    ...
    _r1 = return_mapping(model_1d, deps_fwd, jnp.zeros(1), state0)
    sigma1, state1 = _r1.stress, _r1.state
