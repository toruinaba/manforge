...
from manforge.models.yu_kinematic import YUKinematic1D, YUKinematic3D, YUKinematicPS
from manforge.simulation.integrator import PythonNumericalIntegrator, PythonAnalyticalIntegrator, PythonIntegrator
...
print(80*"=")
print("Numerical calculation(PS)")
start = datetime.now()
res_psn = d_psn.run(hist)
end = datetime.now()
interval = end - start
print(f"Total time: {interval}")
print(80*"=")
...
hist_1dn = [r.n_iterations for r in res_psn.step_results]
...
ax1.plot(res_psn.strain[:, idx], res_psn.stress[:, idx], ":g", label="NumericalPS")
ax2.plot(range(len(hist_n)), hist_n, "-r", label="Numerical3d")
ax2.plot(range(len(hist_a)), hist_a, "--b", label="Analytical3d")
ax2.plot(range(len(hist_1dn)), hist_1dn, ":g", label="NumericalPS")
plt.legend()
plt.show()
