...
def test_mandel_inner_product_equals_double_contraction():
    ...
    a = anp.array([1.0, 2.0, 3.0, 0.0, 0.0, 0.0])
    b = anp.array([4.0, 5.0, 6.0, 0.0, 0.0, 0.0])
    # A:B = 1*4 + 2*5 + 3*6 = 32
    np.testing.assert_allclose(float(anp.dot(to_mandel(a), to_mandel(b))), 32.0)
