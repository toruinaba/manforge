...
## Step 9 — Docker + 基本 Fortran サブルーチン f2py 連携

**目的:** gfortran 環境を Docker で構築し、f2py の基本動作を確認する。

**作成ファイル:**
- `Dockerfile` — gfortran + Python 3.12 + f2py 環境
- `docker-compose.yml` (または Makefile) — ビルド・テスト実行の自動化
- `fortran/test_basic.f90` — 簡単な Fortran サブルーチン (弾性応力計算など)
- `tests/test_fortran_basic.py` — f2py 経由で呼び出し、Python 結果と一致を確認

**検証:** Docker 内で `pytest tests/test_fortran_basic.py -v`

---

## Step 10 — UMAT 模擬サブルーチン + Python クロス検証

**目的:** ABAQUS UMAT インタフェースを模擬し、Python 側 J2 モデルと結果比較。
UMAT は ABAQUS 内部ルーチン (SINV, SPRINC, ROTSIG 等) に依存するため、スタブが必要。

**作成ファイル:**
- `fortran/abaqus_stubs.f90` — ABAQUS 内部関数のモック実装
- `fortran/umat_j2.f90` — J2 等方硬化の Fortran UMAT 実装
- `src/manforge/verification/fortran_bridge.py` (更新) — UMAT ラッパーを強化
- `tests/test_fortran_umat.py` — fortran_bridge 経由で UMAT 呼び出し、Python return_mapping と比較

**検証:** Docker 内で弾性域・塑性域の応力・DDSDDE が Python 実装と rtol=1e-6 で一致
