...
- **多次元対応** — 3D ソリッド / 平面ひずみ / 平面応力 / 1D (NTENS=6/4/3/1) に `StressDimension` で統一対応
...
# 第1引数に StressDimension を渡すと要素タイプを切り替えられる
...
- `autodiff/operators.py` — StressDimension 対応の `dev`, `vonmises`, `hydrostatic`, `norm_mandel`
...
- **Voigt 記法**: 3D ソリッドは `[σ11, σ22, σ33, σ12, σ13, σ23]` (6成分)。次元は `StressDimension` で制御。剪断成分には内部演算で Mandel スケーリング (×√2) を適用
