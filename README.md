...
from manforge.core.stress_state import SOLID_3D, StressState  # 必要に応じて
...
class MyModel(MaterialModel):
    ...
    def __init__(self, stress_state: StressState = SOLID_3D):
        self.stress_state = stress_state  # 省略時は 3D ソリッド (NTENS=6)

    def elastic_stiffness(self, params):
        # self.stress_state に応じた ntens×ntens 弾性剛性テンソル (Voigt)
        return self.isotropic_C(lam, mu)

    def yield_function(self, stress, state, params):
        # f(σ, q) <= 0 で弾性域
        # ss を渡すと任意次元の応力ベクトルに対応
        return vonmises(stress, self.stress_state) - (params["sigma_y0"] + ...)
...
# 平面ひずみ / 軸対称 (NTENS=4)
model_pe = J2IsotropicHardening(stress_state=PLANE_STRAIN)
# → elastic_stiffness が 4×4、return mapping も 4 成分で動作
```
...
`J2IsotropicHardening` では両メソッドが実装済み (J2 radial return 閉形式 + `D^ep` の既知式)。
...
class J2KinematicHardening(MaterialModel):
    ...
    def yield_function(self, stress, state, params):
        ...
        return vonmises(xi, ss) - (params["sigma_y0"] + params["H_iso"] * state["ep"])

    def hardening_increment(self, dlambda, stress, state, params):
        ss = self.stress_state
        # flow direction を JAX 自動微分で取得 (Mandel 重み付け自動適用)
        n = jax.grad(lambda s: vonmises(s - state["alpha"], ss))(stress)
