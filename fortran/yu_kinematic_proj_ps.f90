! =============================================================================
! manforge -- YUKinematicProjPS UMAT (Yoshida-Uemori, projected stagnation)
!
! Fortran port of YUKinematicProjPS (src/manforge/models/yu_kinematic_proj.py).
...
!
! Difference from yu_kinematic_ps.f90 -- the ONLY one
! ---------------------------------------------------
! The stagnation surface is updated by projecting beta onto it instead of
! solving the published mu equation by inner Newton:
!
!     delta_r = h * g_stag
!     delta_q = (1-h) * g_stag * g_xi / ||g_xi||
!
! which leaves ||beta_new - q_new|| - r_new = 0 for any h.  The published
! radicand r_n^2 + 6*h*Fn goes negative once beta recedes far enough from the
! centre, so mu loses its real root and no time-increment cutback recovers it
! (Delta-beta shrinks with the increment, r_n does not).  This form has no
! radical and no iteration, so that failure mode does not exist here:
! fail_code=2 is unreachable and yu_projps_inner_mu_newton is absent.
!
! Everything else -- residual, Jacobian, consistent tangent, NR loop, UMAT
! packing -- is a copy of yu_kinematic_ps.f90 with subroutines renamed
! yu_ps_* -> yu_projps_* so both can be linked into one f2py module for
! cross-checking.  Keep the two in step when touching shared parts.
!
...
!   uv run python -m numpy.f2py -c abaqus_stubs.f90 yu_kinematic_proj_ps.f90 -m yu_kinematic_proj_ps
! =============================================================================


! =============================================================================
! yu_projps_pmat  (internal helper, not f2py-callable)
!
! Returns the plane-stress deviatoric metric P.
! =============================================================================
subroutine yu_projps_pmat(P)
    implicit none
...
end subroutine yu_projps_pmat


! =============================================================================
! yu_projps_pmul  (internal helper, not f2py-callable)
!
! Computes y = P @ x for the plane-stress metric.
! =============================================================================
subroutine yu_projps_pmul(x, y)
    implicit none
...
end subroutine yu_projps_pmul


! =============================================================================
! yu_projps_dev_inner  (internal helper, not f2py-callable)
!
! Deviatoric double contraction via the P metric: dev(s):dev(t) = s^T P t.
! =============================================================================
subroutine yu_projps_dev_inner(s, t, val)
    implicit none
    ...
    call yu_projps_pmul(t, Pt)
    val = s(1) * Pt(1) + s(2) * Pt(2) + s(3) * Pt(3)

end subroutine yu_projps_dev_inner


! =============================================================================
! yu_projps_vonmises_norm  (internal helper, not f2py-callable)
!
! Von Mises equivalent norm under the P metric:
!   ||s||_vm = sqrt(1.5 * s^T P s)
! =============================================================================
subroutine yu_projps_vonmises_norm(s, s_norm)
    implicit none
    ...
    call yu_projps_dev_inner(s, s, sPs)
    s_norm = sqrt(1.5d0 * sPs + EPS_SQRT**2)

end subroutine yu_projps_vonmises_norm


! =============================================================================
! yu_projps_smooth_heaviside  (internal helper, not f2py-callable)
!
! Smooth Heaviside step 0.5*(1 + tanh(beta*x/2)) with beta = 500, matching
! manforge.utils.smooth.smooth_heaviside.
! =============================================================================
subroutine yu_projps_smooth_heaviside(x, hv)
    implicit none
...
end subroutine yu_projps_smooth_heaviside


! =============================================================================
! yu_projps_smooth_sqrt  (internal helper, not f2py-callable)
!
! sqrt(x + eps^2), matching manforge.utils.smooth.smooth_sqrt.
! =============================================================================
subroutine yu_projps_smooth_sqrt(x, res)
    implicit none
    double precision, intent(in)  :: x
...
end subroutine yu_projps_smooth_sqrt


! =============================================================================
! yu_projps_elastic_stiffness
!
...
! =============================================================================
subroutine yu_projps_elastic_stiffness(E, nu, eps_eq, Ea, xi_param, C)
    implicit none
...
end subroutine yu_projps_elastic_stiffness


! =============================================================================
! yu_projps_ck  (internal helper, not f2py-callable)
!
! Kinematic hardening rate with the post-reversal branch:
!   C_k = C_1 - (C_1 - C_2) * H(theta_max - (B_bnd - Y))
! =============================================================================
subroutine yu_projps_ck(C_1, C_2, B_bnd, Y, theta_max, C_k)
    implicit none
    ...
    call yu_projps_smooth_heaviside(theta_max - (B_bnd - Y), hv)
    C_k = C_1 - (C_1 - C_2) * hv

end subroutine yu_projps_ck


! =============================================================================
! yu_projps_calc_residual
!
...
! =============================================================================
subroutine yu_projps_calc_residual(E, nu, Y, B_bnd, C_1, C_2, Rsat, k, b_kin, h, Ea, xi_param, &
    ...
    call yu_projps_elastic_stiffness(E, nu, eps_eq_new, Ea, xi_param, C)

    do ii = 1, 3
        xi(ii) = stress_new(ii) - theta_new(ii) - beta_new(ii)
    end do
    call yu_projps_pmul(xi, Pxi)

    a = B_bnd + R_new - Y
    call yu_projps_vonmises_norm(theta_new, theta_bar)
    call yu_projps_ck(C_1, C_2, B_bnd, Y, theta_max_n, C_k)
    call yu_projps_smooth_sqrt(a / theta_bar, sq_a_tb)
    ...
    call yu_projps_dev_inner(xi, xi, xiPxi)
...
end subroutine yu_projps_calc_residual
...
!   yu_projps_fy_*  <-> calc_fy_*  (R_yield  row)
!   yu_projps_fe_*  <-> calc_fe_*  (R_stress row)
!   yu_projps_ft_*  <-> calc_ft_*  (R_theta  row)
!   yu_projps_fb_*  <-> calc_fb_*  (R_beta   row)
! =============================================================================


! =============================================================================
! yu_projps_fy_fs / yu_projps_fy_ft / yu_projps_fy_fb / yu_projps_fy_fl
!
! d(R_yield)/d(sigma) = P @ eta;  d/d(theta) = d/d(beta) = -P @ eta;  d/dl = 0.
! =============================================================================
subroutine yu_projps_fy_fs(stress, theta, beta, jvec)
    implicit none
    ...
    call yu_projps_pmul(eta, jvec)

end subroutine yu_projps_fy_fs


subroutine yu_projps_fy_ft(stress, theta, beta, jvec)
    implicit none
    double precision, intent(in)  :: stress(3), theta(3), beta(3)
    double precision, intent(out) :: jvec(3)
    ...
    call yu_projps_fy_fs(stress, theta, beta, jvec)
    do ii = 1, 3
        jvec(ii) = -jvec(ii)
    end do

end subroutine yu_projps_fy_ft


subroutine yu_projps_fy_fb(stress, theta, beta, jvec)
    implicit none
    double precision, intent(in)  :: stress(3), theta(3), beta(3)
    double precision, intent(out) :: jvec(3)

    call yu_projps_fy_ft(stress, theta, beta, jvec)

end subroutine yu_projps_fy_fb


subroutine yu_projps_fy_fl(jval)
    implicit none
    double precision, intent(out) :: jval

    jval = 0.0d0

end subroutine yu_projps_fy_fl


! =============================================================================
! yu_projps_dc_deq  (internal helper, not f2py-callable)
!
...
! =============================================================================
subroutine yu_projps_dc_deq(E, Ea, xi_param, eps_eq, C, eta, dlambda, dC_deq, Peta)
    implicit none
    ...
    call yu_projps_pmul(eta, Peta)
    call yu_projps_dev_inner(eta, eta, etaPeta)
    call yu_projps_smooth_sqrt(2.0d0 / 3.0d0 * etaPeta, sq)
...
end subroutine yu_projps_dc_deq
...
! =============================================================================
subroutine yu_projps_fe_fs(E, nu, Ea, xi_param, eps_eq, stress, theta, beta, dlambda, jmat)
    implicit none
    ...
    call yu_projps_elastic_stiffness(E, nu, eps_eq, Ea, xi_param, C)
    call yu_projps_pmat(P)
    do ii = 1, 3
        eta(ii) = stress(ii) - theta(ii) - beta(ii)
    end do
    call yu_projps_dc_deq(E, Ea, xi_param, eps_eq, C, eta, dlambda, dC_deq, Peta)
...
end subroutine yu_projps_fe_fs


! =============================================================================
! yu_projps_fe_ft / yu_projps_fe_fb
!
! d(R_stress)/d(theta) = d/d(beta) = -dl*C@P - dl*outer(dC_deq, P@eta)
! =============================================================================
subroutine yu_projps_fe_ft(E, nu, Ea, xi_param, eps_eq, stress, theta, beta, dlambda, jmat)
    implicit none
    double precision, intent(in)  :: E, nu, Ea, xi_param, eps_eq
    double precision, intent(in)  :: stress(3), theta(3), beta(3), dlambda
    double precision, intent(out) :: jmat(3,3)
    ...
    call yu_projps_fe_fs(E, nu, Ea, xi_param, eps_eq, stress, theta, beta, dlambda, jmat)
    ! fe_fs = I + M  =>  fe_ft = -M
    do jj = 1, 3
        jmat(jj,jj) = jmat(jj,jj) - 1.0d0
    end do
...
end subroutine yu_projps_fe_ft


subroutine yu_projps_fe_fb(E, nu, Ea, xi_param, eps_eq, stress, theta, beta, dlambda, jmat)
    implicit none
    double precision, intent(in)  :: E, nu, Ea, xi_param, eps_eq
    double precision, intent(in)  :: stress(3), theta(3), beta(3), dlambda
    double precision, intent(out) :: jmat(3,3)

    call yu_projps_fe_ft(E, nu, Ea, xi_param, eps_eq, stress, theta, beta, dlambda, jmat)

end subroutine yu_projps_fe_fb
...
!   dC_dl  = fb/f * deq_dl * C
! =============================================================================
subroutine yu_projps_fe_fl(E, nu, Ea, xi_param, eps_eq, stress, theta, beta, dlambda, jvec)
    implicit none
    double precision, intent(in)  :: E, nu, Ea, xi_param, eps_eq
    double precision, intent(in)  :: stress(3), theta(3), beta(3), dlambda
    double precision, intent(out) :: jvec(3)
    ...
    f  = 1.0d0 - (1.0d0 - Ea / E) * (1.0d0 - exp(-xi_param * eps_eq))
    call yu_projps_dev_inner(eta, eta, etaPeta)
    call yu_projps_smooth_sqrt(2.0d0 / 3.0d0 * etaPeta, deq_dl)
...
end subroutine yu_projps_fe_fl


! =============================================================================
! yu_projps_ft_fs / yu_projps_ft_fb
!
! d(R_theta)/d(sigma) = -2/3*C_k*a*dl*I
! d(R_theta)/d(beta)  = +2/3*C_k*a*dl*I
! =============================================================================
subroutine yu_projps_ft_fs(B_bnd, Y, C_1, C_2, R_new, theta_max, dlambda, jmat)
    implicit none
    ...
    a = B_bnd + R_new - Y
    call yu_projps_ck(C_1, C_2, B_bnd, Y, theta_max, C_k)
...
end subroutine yu_projps_ft_fs


subroutine yu_projps_ft_fb(B_bnd, Y, C_1, C_2, R_new, theta_max, dlambda, jmat)
    implicit none
    double precision, intent(in)  :: B_bnd, Y, C_1, C_2, R_new, theta_max, dlambda
    double precision, intent(out) :: jmat(3,3)
    ...
    call yu_projps_ft_fs(B_bnd, Y, C_1, C_2, R_new, theta_max, dlambda, jmat)
    do jj = 1, 3
        do ii = 1, 3
            jmat(ii,jj) = -jmat(ii,jj)
        end do
...
end subroutine yu_projps_ft_fb
...
! =============================================================================
subroutine yu_projps_ft_ft(B_bnd, Y, C_1, C_2, R_new, theta, theta_max, dlambda, jmat)
    implicit none
    ...
    a = B_bnd + R_new - Y
    call yu_projps_ck(C_1, C_2, B_bnd, Y, theta_max, C_k)
    call yu_projps_vonmises_norm(theta, theta_bar)
    call yu_projps_smooth_sqrt(a / theta_bar, sq_a_tb)
    ...
    call yu_projps_pmul(theta, Pth)
    call yu_projps_dev_inner(theta, theta, thPth)
    call yu_projps_smooth_sqrt(thPth, sq_thPth)
    do ii = 1, 3
...
end subroutine yu_projps_ft_ft
...
! =============================================================================
subroutine yu_projps_ft_fl(B_bnd, Y, k, Rsat, C_1, C_2, &
    ...
    call yu_projps_smooth_sqrt(a / theta_bar, sq_a_tb)
    call yu_projps_smooth_sqrt(1.0d0 / a / theta_bar, sq_inv)
...
end subroutine yu_projps_ft_fl


! =============================================================================
! yu_projps_fb_fs / yu_projps_fb_ft / yu_projps_fb_fb / yu_projps_fb_fl
!
...
! =============================================================================
subroutine yu_projps_fb_fs(Y, k, b_kin, dlambda, jmat)
    implicit none
...
end subroutine yu_projps_fb_fs


subroutine yu_projps_fb_ft(Y, k, b_kin, dlambda, jmat)
    implicit none
    double precision, intent(in)  :: Y, k, b_kin, dlambda
    double precision, intent(out) :: jmat(3,3)
    ...
    call yu_projps_fb_fs(Y, k, b_kin, dlambda, jmat)
    do jj = 1, 3
        do ii = 1, 3
            jmat(ii,jj) = -jmat(ii,jj)
        end do
...
end subroutine yu_projps_fb_ft


subroutine yu_projps_fb_fb(Y, k, b_kin, dlambda, jmat)
    implicit none
    double precision, intent(in)  :: Y, k, b_kin, dlambda
    double precision, intent(out) :: jmat(3,3)
...
end subroutine yu_projps_fb_fb


subroutine yu_projps_fb_fl(Y, k, b_kin, stress, theta, beta, jvec)
    implicit none
...
end subroutine yu_projps_fb_fl


! =============================================================================
! yu_projps_calc_jacobian
!
! Assembles the 10x10 Newton-Raphson Jacobian.
!   rows/cols: stress(1..3), dlambda(4), theta(5..7), beta(8..10)
! =============================================================================
subroutine yu_projps_calc_jacobian(E, nu, Y, B_bnd, C_1, C_2, Rsat, k, b_kin, h, Ea, xi_param, &
                                stress_new, theta_new, beta_new, R_new, eps_eq_new, &
    ...
    call yu_projps_fe_fs(E, nu, Ea, xi_param, eps_eq_new, stress_new, theta_new, beta_new, dlambda, Rs_s)
    call yu_projps_fe_ft(E, nu, Ea, xi_param, eps_eq_new, stress_new, theta_new, beta_new, dlambda, Rs_t)
    call yu_projps_fe_fb(E, nu, Ea, xi_param, eps_eq_new, stress_new, theta_new, beta_new, dlambda, Rs_b)
    call yu_projps_fe_fl(E, nu, Ea, xi_param, eps_eq_new, stress_new, theta_new, beta_new, dlambda, Rs_l)

    call yu_projps_ft_fs(B_bnd, Y, C_1, C_2, R_new, theta_max_n, dlambda, Rt_s)
    call yu_projps_ft_ft(B_bnd, Y, C_1, C_2, R_new, theta_new, theta_max_n, dlambda, Rt_t)
    call yu_projps_ft_fb(B_bnd, Y, C_1, C_2, R_new, theta_max_n, dlambda, Rt_b)
    call yu_projps_ft_fl(B_bnd, Y, k, Rsat, C_1, C_2, &
                     stress_new, theta_new, beta_new, R_new, R_n, theta_max_n, dlambda, Rt_l)

    call yu_projps_fb_fs(Y, k, b_kin, dlambda, Rb_s)
    call yu_projps_fb_ft(Y, k, b_kin, dlambda, Rb_t)
    call yu_projps_fb_fb(Y, k, b_kin, dlambda, Rb_b)
    call yu_projps_fb_fl(Y, k, b_kin, stress_new, theta_new, beta_new, Rb_l)

    call yu_projps_fy_fs(stress_new, theta_new, beta_new, Ry_s)
    call yu_projps_fy_ft(stress_new, theta_new, beta_new, Ry_t)
    call yu_projps_fy_fb(stress_new, theta_new, beta_new, Ry_b)
    call yu_projps_fy_fl(Ry_l)
...
end subroutine yu_projps_calc_jacobian


! =============================================================================
! yu_projps_solve  (internal helper, not f2py-callable)
!
...
! =============================================================================
subroutine yu_projps_solve(N, A, LDB, B, NRHS, info)
    implicit none
...
end subroutine yu_projps_solve


! =============================================================================
! yu_projps_calc_ddsdde
!
...
! Parameters
! ----------
! (12 props + converged state, as yu_projps_calc_jacobian)
! eps_eq_n     [in]  : step-start eps_eq, for C_n
! ddsdde(3,3)  [out] : consistent tangent
! =============================================================================
subroutine yu_projps_calc_ddsdde(E, nu, Y, B_bnd, C_1, C_2, Rsat, k, b_kin, h, Ea, xi_param, &
                              stress_new, theta_new, beta_new, R_new, eps_eq_new, &
    ...
    call yu_projps_solve(3, C_work, 3, C_inv, 3, info_lu)
    if (info_lu /= 0) then
        do jj = 1, 3
            do ii = 1, 3
    ...
    call yu_projps_solve(10, jac, 10, rhs10, 10, info_lu)
    if (info_lu /= 0) then
        do jj = 1, 3
            do ii = 1, 3
                ddsdde(ii,jj) = C_n(ii,jj)
...
end subroutine yu_projps_calc_ddsdde
...
!                                        3 = linear solve failed.  Code 2 (mu
!                                        Newton) exists in the published port
!                                        and is unreachable here; the numbering
!                                        is kept so logs from both are readable
!                                        with the same key.
! fail_diag(6)                   [out] : state at the failing iteration:
!                                        [r_n, g_stag, 0, 0, dbeta_norm,
!                                        dlambda].  Slots 3-4 carried Gn and the
!                                        mu radicand in the published port and
!                                        have no counterpart here.  Zero on
!                                        success.
! =============================================================================
subroutine yu_kinematic_proj_ps( &
        E, nu, Y, B_bnd, C_1, C_2, Rsat, k, b_kin, h, Ea, xi_param, &
        stress_n, &
    ...
    double precision :: delta_q(3), delta_rstag, delta_Rbnd
    double precision :: theta_norm_final, dbeta_norm
    integer :: ii, jj, iter, info_lu
    ...
    ! ---- elastic predictor -------------------------------------------------
    call yu_projps_elastic_stiffness(E, nu, eps_eq_n, Ea, xi_param, C)
    do ii = 1, 3
    ...
    call yu_projps_dev_inner(xi_trial, xi_trial, xiPxi)
    ...
    do iter = 1, 50
        call yu_projps_calc_residual(E, nu, Y, B_bnd, C_1, C_2, Rsat, k, b_kin, h, Ea, xi_param, &
        ...
        call yu_projps_solve(10, jac, 10, dx, 1, info_lu)
        if (info_lu /= 0) then
            converged = 0
            n_iter    = iter
            fail_code = 3
        ...
        call yu_projps_dev_inner(eta, eta, g)
        call yu_projps_smooth_sqrt(2.0d0 / 3.0d0 * g, delta_eps_eq)
        ...
        call yu_projps_vonmises_norm(g_xi, stag_norm)
        g_stag = stag_norm - rstag_n
        call yu_projps_smooth_heaviside(g_stag + 1.0d-10, g_flag)

        ! Projected stagnation update: move the centre by (1-h)*g_stag along
        ! g_xi and grow the radius by h*g_stag, which leaves beta exactly on the
        ! updated surface for any h.  No iteration and no radical, so the
        ! negative-radicand failure of the published mu equation cannot occur --
        ! fail_code=2 is unreachable in this file.
        delta_rstag = h * g_stag
        do ii = 1, 3
            delta_q(ii) = (1.0d0 - h) * g_stag * g_xi(ii) / stag_norm
        end do
    ...
    ! Exhausting the loop leaves n_iter at its initial 0; 50 marks that, versus
    ! the early exit on a failed linear solve.  fail_code=2 (mu) cannot occur
    ! here -- there is no mu iteration to fail.
    ! fail_diag slots 2..4 held Fn / Gn / the mu radicand in the published port;
    ! none of those exist in the projected form, so g_stag takes slot 2 (it is
    ! what drives the whole update) and 3..4 stay zero.
    if (converged == 0 .and. n_iter == 0) then
        n_iter    = 50
        fail_code = 1
        call yu_projps_dev_inner(d_beta, d_beta, dbeta_norm)
    ...
    call yu_projps_vonmises_norm(theta_new, theta_norm_final)
    ...
    call yu_projps_calc_ddsdde(E, nu, Y, B_bnd, C_1, C_2, Rsat, k, b_kin, h, Ea, xi_param, &
                           stress_new, theta_new, beta_new, Rbnd_new, eps_eq_new, &
                           theta_max_n, Rbnd_n, dlambda, eps_eq_n, ddsdde)

end subroutine yu_kinematic_proj_ps
...
! calls yu_kinematic_proj_ps.  Non-convergence is signalled to ABAQUS via
...
subroutine umat(STRESS, STATEV, DDSDDE, SSE, SPD, SCD, &
    ...
    if (converged == 0) then
        ...
        call yu_projps_elastic_stiffness( &
        ...
        if (YU_DEBUG) then
            ...
            if (fail_code == 1) then
                ...
                ! No mu iteration exists in this port, so every non-NR failure
                ! is the linear solve; yu_nmu stays 0 and is kept only so the
                ! YU-NC columns line up with the published port's log.
                yu_nlu = yu_nlu + 1
            end if

            ! Per-point detail, capped (grep "YU-FL"):
            !   YU-FL  code  noel  npt  n_iter  r_n  g_stag  --  --  |dbeta|  dlambda
            ! code: 1=NR exhausted, 3=linear solve.  (2=mu is unreachable here.)
            ! Slots 3 and 4 are always zero: they carried Gn and the mu radicand
            ! in the published port, neither of which the projection forms.
