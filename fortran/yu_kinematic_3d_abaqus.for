! ======================================================================
! manforge -- YUKinematic3D UMAT (Yoshida-Uemori 2-surface + stagnation 
!
...
!   - The output array argument "J" in Python is named "jmat" (6x6), "jv
!     (6-vector), or "jval" (scalar) to avoid collision with the Fortran
!     variable j.
!
...
!   PROPS(9)  = b        backstress (beta) hardening rate  (Python: self
!   PROPS(10) = h        stagnation surface parameter
!   PROPS(11) = Ea       asymptotic modulus (nonlinear elasticity)
!   PROPS(12) = xi_param nonlinear elasticity decay parameter (Python: s
!
! State variables (STATEV, 22 slots, model.state_names without "stress")
!   STATEV(1..6)   = theta(1..6)     relative backstress of yield surfac
!   STATEV(7..12)  = beta(1..6)      relative backstress of boundary sur
!   STATEV(13)     = R               radius increment of boundary surfac
...
!   uv run python -m numpy.f2py -c abaqus_stubs.f90 yu_kinematic_3d.f90 
! ======================================================================
...
! ======================================================================
      subroutine yu_kinematic_3d_elastic_stiffness(E, nu, eps_eq, Ea,
     &xi_param, C)
      implicit none
      real*8, intent(in)  :: E, nu, eps_eq, Ea, xi_param
      real*8, intent(out) :: C(6,6)

      real*8 :: lam, mu, factor
      integer :: ii, jj

      mu     = E / (2.0d0 * (1.0d0 + nu))
      lam    = E * nu / ((1.0d0 + nu) * (1.0d0 - 2.0d0 * nu))
      factor = 1.0d0 - (1.0d0 - Ea / E) * (1.0d0 - exp(-xi_param *
     &eps_eq))
...
! ======================================================================
      ...
      implicit none
      real*8, intent(in)  :: xi(6)
      real*8, intent(out) :: xi_norm

      real*8 :: sss
      real*8, parameter :: EPS_SQRT = 1.0d-12
      integer :: ii
...
! ======================================================================
      ...
      implicit none
      real*8, intent(in)  :: xi(6)
      real*8, intent(out) :: xi_norm, flow(6)
...
! yu_prepare_rstress
!
! Computes the partial derivative of the plastic flow direction n with r
...
! ======================================================================
      ...
      implicit none
      real*8, intent(in)  :: xi(6)
      real*8, intent(out) :: dn_dsig(6,6)

      real*8 :: xi_norm, flow(6)
      real*8 :: n_hat(6), T_I_dev(6,6), P_ij
      real*8, parameter :: SQRT15 = 1.2247448713915890d0  ! sq
      integer :: ii, jj
...
! I_dev_ij = delta_ij - 1/3 * [i<=3 and j<=3]
      ...
      do ii = 1, 6
      do jj = 1, 6
      dn_dsig(ii,jj) = 1.5d0 / xi_norm * (T_I_dev(ii,
     &jj) - n_hat(ii) * n_hat(jj))
      end do
      end do
...
! ======================================================================
      subroutine yu_prepare_rtheta(B_bnd, Y, k, Rsat, C_1, C_2, theta,
     &theta_max, R, R_n, dlambda, theta_bar, theta_flow, C_k, s, a,
     &a_prime)
      implicit none
      real*8, intent(in)  :: B_bnd, Y, k, Rsat, C_1, C_2
      real*8, intent(in)  :: theta(6), theta_max, R, R_n,
     &dlambda
      real*8, intent(out) :: theta_bar, theta_flow(6), C_k, s,
     &a, a_prime
...
! smooth_heaviside (consistent with residual): matches Python _prepare_R
...
! ======================================================================
      ...
      implicit none
      real*8, intent(in)  :: C(6,6), xi(6), dlambda
      real*8, intent(out) :: jmat(6,6)
...
! yu_dflow_dxi
!
! d(flow)/d(xi)  (without I_dev projection; use for dRs/dtheta and dRs/d
!
! flow = 1.5*T@xi / |xi|_VM,  d(flow)/d(xi) = 1.5/|xi| * (T - outer(n_ha
! where n_hat = flow/sqrt(1.5)  (unit-norm direction).
!
! Unlike yu_prepare_rstress (which computes d(flow)/d(xi) @ I_dev for dR
...
! ======================================================================
      ...
      implicit none
      real*8, intent(in)  :: xi(6)
      real*8, intent(out) :: dflow(6,6)

      real*8 :: xi_norm, flow(6), n_hat(6)
      real*8, parameter :: SQRT15 = 1.2247448713915890d0
      integer :: ii, jj
...
!
! d(xi)/d(beta) = -I (no deviatoric projection), so use yu_dflow_dxi wit
! ======================================================================
...
! yu_dRs_dtheta
!
! dR_stress / d_theta = -dlambda * C @ dflow_dxi  (identical to dRs_dbet
!
! d(xi)/d(theta) = -I (no deviatoric projection), so use yu_dflow_dxi wi
! ======================================================================
...
!     - xi_param * (1 - Ea/E) * exp(-xi_param*eps_eq) / factor * dlambda
...
! ======================================================================
      subroutine yu_dRs_dlambda(E, Ea, xi_param, C, xi, eps_eq, dlambda,
     &jvec)
      implicit none
      real*8, intent(in)  :: E, Ea, xi_param
      real*8, intent(in)  :: C(6,6), xi(6), eps_eq, dlambda
      real*8, intent(out) :: jvec(6)

      real*8 :: xi_norm, flow(6), Cn(6), factor, exp_term
      integer :: ii, kk
      ...
      do ii = 1, 6
      jvec(ii) = Cn(ii) - xi_param * (1.0d0 - Ea / E) * exp_term /
     &factor * dlambda * Cn(ii)
      end do
...
! ======================================================================
      ...
      implicit none
      real*8, intent(in)  :: k, b_kin, Y, dlambda
      real*8, intent(out) :: jmat(6,6)
...
! ======================================================================
      ...
      implicit none
      real*8, intent(in)  :: k, b_kin, Y
      real*8, intent(in)  :: xi(6), beta(6), dlambda
      real*8, intent(out) :: jvec(6)
...
! ======================================================================
      subroutine yu_dRt_dstress(B_bnd, Y, k, Rsat, C_1, C_2, theta,
     &theta_max, R, R_n, dlambda, jmat)
      implicit none
      real*8, intent(in)  :: B_bnd, Y, k, Rsat, C_1, C_2
      real*8, intent(in)  :: theta(6), theta_max, R, R_n,
     &dlambda
      ...
      real*8 :: theta_bar, theta_flow(6), C_k, s, a, a_prime
      real*8 :: coeff, P_ij
      integer :: ii, jj

      call yu_prepare_rtheta(B_bnd, Y, k, Rsat, C_1, C_2, theta,
     &theta_max, R, R_n, dlambda, theta_bar, theta_flow, C_k, s, a,
     &a_prime)
...
! ======================================================================
      subroutine yu_dRt_dbeta(B_bnd, Y, k, Rsat, C_1, C_2, theta,
     &theta_max, R, R_n, dlambda, jmat)
      implicit none
      real*8, intent(in)  :: B_bnd, Y, k, Rsat, C_1, C_2
      real*8, intent(in)  :: theta(6), theta_max, R, R_n,
...
!
! dR_theta / d_theta.
! Has a theta_bar < 1e-14 guard to avoid division by zero (Python line 2
! ======================================================================
      subroutine yu_dRt_dtheta(B_bnd, Y, k, Rsat, C_1, C_2, theta,
     &theta_max, R, R_n, dlambda, jmat)
      implicit none
      real*8, intent(in)  :: B_bnd, Y, k, Rsat, C_1, C_2
      real*8, intent(in)  :: theta(6), theta_max, R, R_n,
...
! Degenerate case: only diagonal term (Python line 276)
      ...
      diag_coeff  = 1.0d0 + a * C_k * dlambda / Y + C_k * dlambda *
     &sqrt(a / theta_bar)
      outer_coeff = SQRT15 * C_k * dlambda * sqrt(a / theta_bar) /
     &(2.0d0 * theta_bar)
      do ii = 1, 6
      do jj = 1, 6
...
! ======================================================================
      subroutine yu_dRt_dlambda(B_bnd, Y, k, Rsat, C_1, C_2, xi, theta,
     &theta_max, R, R_n, dlambda, jvec)
      implicit none
      real*8, intent(in)  :: B_bnd, Y, k, Rsat, C_1, C_2
      real*8, intent(in)  :: xi(6), theta(6), theta_max, R,
     &R_n, dlambda
      real*8, intent(out) :: jvec(6)
...
! Degenerate case (Python line 284)
      ...
      do ii = 1, 6
      jvec(ii) = - a * C_k / Y * xi(ii) - C_k * dlambda / Y * a_prime *
     &xi(ii) + C_k * sqrt(a / theta_bar) * theta(ii) + C_k * dlambda *
     &sqrt(1.0d0 / (theta_bar * a)) * a_prime / 2.0d0 * theta(ii)
      end do
      end if
...
! yu_calc_residual
!
! Computes the full 19-element residual vector for the Newton-Raphson sy
! r = [R_stress(6), R_yield(1), R_theta(6), R_beta(6)]
!
! Note on C_k: uses smooth_heaviside (residual side), unlike the Jacobia
! which uses a hard step via _prepare_Rtheta. Matches Python calc_residu
!
! Parameters
! ----------
! E, nu, Y, B_bnd, C_1, C_2, Rsat, k, b_kin, h, Ea, xi_param
!             : 12 material parameters (order = model.param_names)
! stress_new(6), theta_new(6), beta_new(6)  : NR iterates (Implicit stat
...
! ======================================================================
      subroutine yu_calc_residual(E, nu, Y, B_bnd, C_1, C_2, Rsat, k,
     &b_kin, h, Ea, xi_param, stress_new, theta_new, beta_new, R_new,
     &eps_eq_new, theta_n, beta_n, theta_max_n, stress_trial, dlambda,
     &r_vec)
      implicit none
      real*8, intent(in)  :: E, nu, Y, B_bnd, C_1, C_2, Rsat,
     &k, b_kin, h, Ea, xi_param
      real*8, intent(in)  :: stress_new(6), theta_new(6),
     &beta_new(6)
      real*8, intent(in)  :: R_new, eps_eq_new
      real*8, intent(in)  :: theta_n(6), beta_n(6),
     &theta_max_n
      real*8, intent(in)  :: stress_trial(6), dlambda
      real*8, intent(out) :: r_vec(19)

      real*8 :: C(6,6), dev_stress(6), xi(6)
      real*8 :: xi_norm, flow(6), theta_norm
      real*8 :: C_k, hv, a, sm_sqrt_result
      real*8 :: R_stress(6), R_theta(6), R_beta(6), R_yield
      integer :: ii, kk

! Elastic stiffness
      call yu_kinematic_3d_elastic_stiffness(E, nu, eps_eq_new, Ea,
     &xi_param, C)
...
!         - (C_k*a/Y*xi - C_k*smooth_sqrt(a/theta_norm)*theta_new)*dlamb
      call yu_smooth_sqrt(a / theta_norm, sm_sqrt_result)
      do ii = 1, 6
      R_theta(ii) = theta_new(ii) - theta_n(ii) - (C_k * a / Y * xi(ii)
     &- C_k * sm_sqrt_result * theta_new(ii)) * dlambda
      end do

! R_beta = beta_new - beta_n - (k*b_kin/Y*xi - k*beta_new)*dlambda
      do ii = 1, 6
      R_beta(ii) = beta_new(ii) - beta_n(ii) - (k * b_kin / Y * xi(ii)
     &- k * beta_new(ii)) * dlambda
      end do
...
! Row order:    R_stress(1..6), R_yield(7), R_theta(8..13), R_beta(14..1
!
! Parameters
! ----------
! E, nu, Y, B_bnd, C_1, C_2, Rsat, k, b_kin, h, Ea, xi_param
...
! ======================================================================
      subroutine yu_calc_jacobian(E, nu, Y, B_bnd, C_1, C_2, Rsat, k,
     &b_kin, h, Ea, xi_param, stress_new, theta_new, beta_new, R_new,
     &eps_eq_new, theta_max_new, R_n, dlambda, jac)
      implicit none
      real*8, intent(in)  :: E, nu, Y, B_bnd, C_1, C_2, Rsat,
     &k, b_kin, h, Ea, xi_param
      real*8, intent(in)  :: stress_new(6), theta_new(6),
     &beta_new(6)
      real*8, intent(in)  :: R_new, eps_eq_new, theta_max_new,
     &R_n, dlambda
      real*8, intent(out) :: jac(19,19)

      real*8 :: C(6,6), dev_stress(6), xi(6)
      real*8 :: Rs_s(6,6), Rs_b(6,6), Rs_t(6,6), Rs_l(6)
      real*8 :: Rb_s(6,6), Rb_b(6,6), Rb_t(6,6), Rb_l(6)
      real*8 :: Rt_s(6,6), Rt_b(6,6), Rt_t(6,6), Rt_l(6)
      real*8 :: Rl_s(6), Rl_b(6), Rl_t(6), Rl_l
      integer :: ii, jj
...
! Rstress blocks
      ...
      call yu_dRs_dlambda(E, Ea, xi_param, C, xi, eps_eq_new, dlambda,
     &Rs_l)
...
! Rtheta blocks
      call yu_dRt_dstress(B_bnd, Y, k, Rsat, C_1, C_2, theta_new,
     &theta_max_new, R_new, R_n, dlambda, Rt_s)
      call yu_dRt_dbeta(B_bnd, Y, k, Rsat, C_1, C_2, theta_new,
     &theta_max_new, R_new, R_n, dlambda, Rt_b)
      call yu_dRt_dtheta(B_bnd, Y, k, Rsat, C_1, C_2, theta_new,
     &theta_max_new, R_new, R_n, dlambda, Rt_t)
      call yu_dRt_dlambda(B_bnd, Y, k, Rsat, C_1, C_2, xi, theta_new,
     &theta_max_new, R_new, R_n, dlambda, Rt_l)
...
! Rows 1..6: R_stress; cols: stress(1..6), dlambda(7), theta(8..13), bet
      do ii = 1, 6
      do jj = 1, 6
...
! ======================================================================
! solve19 -- in-place LU solver for a 19x19 system with NRHS right-hand 
!
...
! floating-point behaviour across compilers (gfortran vs Intel Fortran).
      ...
      real*8, intent(inout) :: A(19,19), B(19,NRHS)
...
! ======================================================================
      ...
      implicit none
      real*8, intent(in)  :: h, r_n, Gn, Fn
      real*8, intent(out) :: mu_out
      integer,          intent(out) :: info

      integer :: i
      real*8 :: mu, H_mu, F_mu, F_mu_prime
      real*8, parameter :: EPS_SQRT = 1.0d-12

      mu = 0.0d0
      info = 0  ! default: converged (mu=0 is returned if r_n is degener
...
! yu_calc_ddsdde
!
! Computes the consistent algorithmic tangent (6x6) from the converged s
...
! ======================================================================
      subroutine yu_calc_ddsdde(E, nu, Y, B_bnd, C_1, C_2, Rsat, k,
     &b_kin, h, Ea, xi_param, stress_new, theta_new, beta_new, R_new,
     &eps_eq_new, theta_max_new, R_n, dlambda, ddsdde, eps_eq_n)
      implicit none
      real*8, intent(in)  :: E, nu, Y, B_bnd, C_1, C_2, Rsat,
     &k, b_kin, h, Ea, xi_param
      real*8, intent(in)  :: stress_new(6), theta_new(6),
     ...
     &R_n, dlambda
      ...
      real*8, intent(in)  :: eps_eq_n  ! step-start eps_eq for

      real*8 :: jac(19,19), C(6,6), C_n(6,6), C_inv(6,6),
     &C_work(6,6)
      real*8 :: rhs19(19,19), jac_stress_block(6,19)
      integer :: ii, jj, kk, info_lu
      real*8, parameter :: ZERO = 0.0d0, ONE = 1.0d0

! Step 1: Build full 19x19 Jacobian (uses eps_eq_new for C in dRs/dlambd
      call yu_calc_jacobian(E, nu, Y, B_bnd, C_1, C_2, Rsat, k, b_kin,
     &h, Ea, xi_param, stress_new, theta_new, beta_new, R_new,
     &eps_eq_new, theta_max_new, R_n, dlambda, jac)

! Step 2: Reconstruct C for Jacobian (eps_eq_new) and C_n for rhs scalin
! C_n (step-start stiffness) is the correct rhs scaling: J*dx/de = [C_n;
! -> ddsdde = J^{-1}[0:6,0:6]*C_n. Using C(state_new) was a bug when E v
      call yu_kinematic_3d_elastic_stiffness(E, nu, eps_eq_new, Ea,
     &xi_param, C)
      call yu_kinematic_3d_elastic_stiffness(E, nu, eps_eq_n,   Ea,
     &xi_param, C_n)
      do ii = 1, 6
      do jj = 1, 6
...
! Thin wrapper around LAPACK dgesv for numerical stability.
      implicit none
      real*8, intent(inout) :: A(6,6), B(6,6)
      integer,          intent(out)   :: info
...
! Implements the full return mapping (elastic predictor + NR plastic cor
...
!   theta(6), beta(6), R(scalar), q(6), r(scalar), eps_eq(scalar), theta
!
! Parameters
! ----------
...
! n_iter                         [out] : number of outer NR iterations p
! converged                      [out] : 1 = converged, 0 = not converge
! ======================================================================
      subroutine yu_kinematic_3d( E, nu, Y, B_bnd, C_1, C_2, Rsat, k,
...
! -- inputs: stress + state at step start
      real*8, intent(in) :: stress_n(6)
      real*8, intent(in) :: theta_n(6), beta_n(6), Rbnd_n,
     &q_n(6), rstag_n, eps_eq_n, theta_max_n
! -- input: strain increment
      real*8, intent(in) :: dstran(6)
! -- outputs: updated stress + state
      real*8, intent(out) :: stress_out(6)
      real*8, intent(out) :: theta_out(6), beta_out(6),
     &Rbnd_out, q_out(6), rstag_out, eps_eq_out, theta_max_out
...
! -- local variables
      ...
      real*8 :: stress_new(6), theta_new(6), beta_new(6)
      real*8 :: Rbnd_new, rstag_new, eps_eq_new
      real*8 :: q_new(6)
      real*8 :: dlambda
     ...
     &delta_Rbnd, s_fac
      ...
      real*8, parameter :: TOL_NR   = 1.0d-10
      real*8, parameter :: EPS_SQRT = 1.0d-12
...
! Residual (theta_max passed as state_n value -- not updated during NR)
      call yu_calc_residual(E, nu, Y, B_bnd, C_1, C_2, Rsat, k, b_kin,
     &h, Ea, xi_param, stress_new, theta_new, beta_new, Rbnd_new,
     &eps_eq_new, theta_n, beta_n, theta_max_n, stress_trial, dlambda,
     &r_vec)
...
! Jacobian and linear solve
      call yu_calc_jacobian(E, nu, Y, B_bnd, C_1, C_2, Rsat, k, b_kin,
     &h, Ea, xi_param, stress_new, theta_new, beta_new, Rbnd_new,
     &eps_eq_new, theta_max_n, Rbnd_n, dlambda, jac)
...
! delta_q, delta_rstag, delta_Rbnd
      ...
      H_mu_fin   = sqrt(rstag_n*rstag_n + 6.0d0*h*Fn / (1.0d0 + mu) +
     &EPS_SQRT**2)
...
! Capture stagnation state at iter=3 for diagnostics (YU-IV output in UM
      ...
      stag_vals(2) = Gn
      stag_vals(3) = g_stag
      stag_vals(4) = Rbnd_new
      stag_vals(5) = eps_eq_new
      stag_vals(6) = dlambda
      end if
...
! ======================================================================
      call yu_calc_ddsdde(E, nu, Y, B_bnd, C_1, C_2, Rsat, k, b_kin, h,
     &Ea, xi_param, stress_new, theta_new, beta_new, Rbnd_new,
     &eps_eq_new, theta_max_n, Rbnd_n, dlambda, ddsdde, eps_eq_n)
...
! STATEV layout: see file header (STATEV(1..6)=theta .. STATEV(22)=theta
! ======================================================================
...
C
     ...
     2 STRAN,DSTRAN,TIME,DTIME,TEMP,DTEMP,PREDEF,DPRED,CMNAME,
     3 NDI,NSHR,NTENS,NSTATV,PROPS,NPROPS,COORDS,DROT,PNEWDT,
     4 CELENT,DFGRD0,DFGRD1,NOEL,NPT,LAYER,KSPT,JSTEP(1),KINC)
      RETURN
      END
C
C =============================================================================
...
C =============================================================================
     ...
     &KSTEP, KINC
      ...
      real*8,     intent(out)   :: SSE, SPD, SCD, RPL, DRPLDT
      real*8,     intent(out)   :: DDSDDT(NTENS),
     &DRPLDE(NTENS)
      real*8,     intent(in)    :: STRAN(NTENS), DSTRAN(NTENS)
      real*8,     intent(in)    :: TIME(2), DTIME, TEMP, DTEMP
      real*8,     intent(in)    :: PREDEF(1), DPRED(1)
      real*8,     intent(in)    :: PROPS(NPROPS), COORDS(3)
      real*8,     intent(in)    :: DROT(3,3), DFGRD0(3,3),
     &DFGRD1(3,3)
      real*8,     intent(inout) :: PNEWDT
      real*8,     intent(in)    :: CELENT

      real*8 :: theta_n(6), beta_n(6), Rbnd_n, q_n(6), rstag_n
      real*8 :: eps_eq_n, theta_max_n
      real*8 :: stress_out(6), theta_out(6), beta_out(6),
     &Rbnd_out
      real*8 :: q_out(6), rstag_out, eps_eq_out, theta_max_out
      real*8 :: ddsdde_local(6,6)
      real*8 :: theta_rot(6), beta_rot(6), q_rot(6)
      real*8 :: theta_out_rot(6), beta_out_rot(6),
     &q_out_rot(6)
...
! Diagnostic: per-increment failure counters (retained across calls via 
      ...
      real*8, save :: yu_time  = 0.0d0
      real*8, save :: yu_dtime = 0.0d0

! Guard: this UMAT is for 3-D solid elements only (NTENS=6, NDI=3, NSHR=
      if (NTENS /= 6 .or. NDI /= 3 .or. NSHR /= 3 .or. NSTATV < 22 .or.
     &NPROPS < 12) then
      write(7,'(A)') 'YUKinematic3D UMAT: incompatible element/material
     &definition.'
      write(7,'(A,I0,A,I0,A,I0)') '  Expected NTENS=6 NDI=3 NSHR=3,
     &got NTENS=', NTENS, ' NDI=', NDI, ' NSHR=', NSHR
      write(7,'(A,I0,A,I0)') '  Expected NSTATV>=22 NPROPS>=12,
     &got NSTATV=', NSTATV, ' NPROPS=', NPROPS
      PNEWDT = 0.0d0
      return
      end if

! STATEV unpack + co-rotate tensor state variables
! STRESS is already co-rotated by ABAQUS before UMAT entry (no ROTSIG ne
! theta, beta, q are deviatoric stress-like tensors stored in STATEV and
! be co-rotated here so the return-mapping operates in the rotated frame
      ...
      call yu_kinematic_3d( PROPS(1), PROPS(2), PROPS(3), PROPS(4),
     &PROPS(5), PROPS(6), PROPS(7), PROPS(8), PROPS(9), PROPS(10),
     &PROPS(11), PROPS(12), STRESS, theta_rot, beta_rot, Rbnd_n, q_rot,
     &rstag_n, eps_eq_n, theta_max_n, DSTRAN, stress_out, theta_out,
     &beta_out, Rbnd_out, q_out, rstag_out, eps_eq_out, theta_max_out,
...
! Previously STRESS/STATEV were overwritten before PNEWDT was set, causi
! the retry to start from a corrupted state (partially-converged values)
!
! Diagnostic: accumulate per-increment failure counts and flush one summ
! line to the msg file when the increment changes.  Format (grep "YU-NC"
!   YU-NC  kstep  kinc     time      dtime   n_fail  n_nr  n_mu
! n_nr: return-mapping NR non-convergence (n_iter==50, all 50 iters cons
...
! Flush previous increment summary when increment changes
      ...
      if (yu_kinc /= -1) then
      write(7,'(A,2I6,2ES11.3,3I8)') 'YU-NC ', yu_kstep, yu_kinc,
     &yu_time, yu_dtime, yu_nfail, yu_nnr, yu_nmu
      end if
      yu_kstep = KSTEP
      yu_kinc  = KINC
      yu_time  = TIME(1)
...
! YU-RQ: Rbnd  rstag  eps_eq  theta_max  vm(q)
      ...
      write(7,'(A,5ES22.14)') 'YU-RQ ', STATEV(13), STATEV(20),
     &STATEV(21), STATEV(22),
...
! compare with external calculation to find discrepancy
      write(7,'(A,ES22.14,A,F10.4)') 'YU-XT ', xi_trial_norm_val, ' Y=',
     &PROPS(3)
! YU-TR/YU-BR: theta/beta after ROTSIG (actual values passed to yu_kinem
! If YU-TR == YU-TH, no rotation occurred; driver test reproduces exact 
...
! Provide elastic DDSDDE so ABAQUS global NR gets a valid stiffness matr
! Without this, DDSDDE is uninitialised, which may corrupt the global NR
      call yu_kinematic_3d_elastic_stiffness( PROPS(1), PROPS(2),
     &eps_eq_n, PROPS(11), PROPS(12), DDSDDE)
...
! theta_out, beta_out, q_out are already in the rotated frame (return ma
