...
- `verification/jacobian.py`: `JacobianBlocks` dataclass and `ad_jacobian_blocks(model, result, state_n)` — computes the residual Jacobian at the converged point and exposes blocks via `jac.part[row_residual_name][col_state_name]`. Row names are residual labels (`effective_residual_name` from the field, or `model.dlambda_residual_name` for the Δλ slot); column names are state names (`"stress"`, `"dlambda"`, implicit non-stress keys). Default (no `residual_name` set): row and column names are identical (e.g. `jac.part["alpha"]["stress"]`). With opt-in: `Implicit(residual_name="R_alpha")` → `jac.part["R_alpha"]["stress"]`. `jac.row_names()` / `jac.col_names()` enumerate canonical name tuples; `jac.iter_blocks()` yields `("row::col", array)` pairs. `jac.full` is the full `(n_unknown, n_unknown)` matrix. Accepts both `StressUpdateResult` and `ReturnMappingResult` (latter requires `stress_trial=` kwarg). Import from `manforge.verification.jacobian`.
